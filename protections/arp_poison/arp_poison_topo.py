...
def apply_static_arp(net):
    """
    Apply static ARP entries directly in Python without relying on shell scripts.

    :param net: Mininet network
    """
    info(">>> Applying static ARP entries to all hosts\n")
    
    
    # arp entries to set (source_host, target_ip, target_host)
    arp_entries = [
        # LAN (s1)
        ('ws2', '10.1.0.1', 'r1'),
        ('ws3', '10.1.0.1', 'r1'),
        ...
        # Internet
        ('internet', '10.2.0.1', 'r2'),
        ('r2', '10.2.0.2', 'internet')
    ]
    
    for src_host, target_ip, target_host in arp_entries:
        target_mac = None
        if target_host in net:
            for intf in net[target_host].intfList():
                if intf.IP() == target_ip:
                    target_mac = intf.MAC()
                    break
    
            if not target_mac and net[target_host].intfList():
                target_mac = net[target_host].intfList()[0].MAC() # if we can't find it
